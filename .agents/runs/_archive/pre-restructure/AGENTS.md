...
<!-- BEGIN:graphify-integration -->
## 7. Codebase Exploration (Graphify)
- A **Graphify** knowledge graph daemon is running in watch mode for this repository.
- **Before making architectural changes or exploring the codebase**, ALWAYS read `graphify-out/GRAPH_REPORT.md` or parse `graphify-out/graph.json` to understand file relationships, communities, and god nodes.
- Do not rely solely on `grep` or file listings if you can query the graph.
- Since it runs in `--watch` mode, any code changes you make will be automatically reflected in the graph. You can trust it is up to date.
<!-- END:graphify-integration -->

<!-- BEGIN:classroom-os-domain-architecture -->
## 8. Domain-Based Architecture & Refactor Laws
- Features are strictly grouped by domain under src/features/[domain]/ (e.g., uth, users, ttendance, ssignments).
- Original route components should cleanly import from these domains using export * stubs to maintain route structure backward compatibility.
- Do NOT duplicate authorization logic across actions. Use the centralized can(user, action, resource) helper from src/lib/auth/rbac.ts.
- AuthorizationResource must be strictly typed using a discriminated union pattern. Do NOT use ny.
- Do NOT introduce frameworks like Auth.js, Prisma, or migrate to PostgreSQL. Use standard Next.js server-side cookies, LibSQL/Turso, and Drizzle.
<!-- END:classroom-os-domain-architecture -->
