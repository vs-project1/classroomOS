...
| **34** | Admin blocked from resource creation and units due to rigid teacher profile checks and empty chapter filtering | `createResourceAction` and `/teacher/resources` enforced `if (!user.teacherId)` despite `requireAuth(["TEACHER", "ADMIN"])`, assuming only teacher accounts upload course materials. Additionally, `ResourceForm` filtered out units with zero chapters (`unit.chapters.length > 0`), hiding all 42 syllabus units. | Treat Admin as the primary superuser who inherits all capabilities across the system; teacher/CR/student views are scoped subsets of admin capabilities. Server actions must allow `ADMIN` to upload resources across all subjects/units with nullable `uploadedBy`, auto-anchor initial chapters under units, and unit dropdowns must render units even when sub-chapters have not yet been defined. |
...
5. **Update Milestone Status:** If this task advances or completes a milestone, update the tracking board in `PROJECTS.md`.
