    ...
    <div className="flex-1 space-y-8 max-w-5xl">
      ...
      <div className="space-y-4 pt-6 border-t border-border/40">
        ...
        {pendingDisputes.length === 0 ? (
          <EmptyState
            className="p-8"
            icon={<CheckCircle2 className="w-8 h-8 text-emerald-500 opacity-80" />}
            title="No pending disputes"
            description="All attendance correction requests have been addressed."
          />
        ) : (
          <div className="space-y-4">
            {pendingDisputes.map((dispute) => (
              <DisputeCard key={dispute.id} req={dispute} showActions />
            ))}
          </div>
        )}
      </div>
