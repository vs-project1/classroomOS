...
/**
 * Grouped student sidebar — sections: ACADEMICS (My Subjects, Timetable, Class History, What You Missed), Work, Class, Campus.
 * Keeps role filtering via getNavigationForRole; defaults to STUDENT.
 */
export function StudentSidebar({ role = "STUDENT", badges, subjects }: StudentSidebarProps) {
      ...
      <nav aria-label="Primary" className="flex-1 px-3 py-4 space-y-4 overflow-y-auto">
        {navigation.sections.map((section, idx) => (
          <div key={section.title ?? `top-${idx}`}>
            {section.title && (
              <p className="px-3 pb-1.5 text-[11px] font-semibold tracking-[0.08em] text-slate-500 uppercase">{section.title}</p>
            )}
                    ...
                    {item.badgeKey && badges && (
                      <>
                        <span className="sr-only">
                          {badges[item.badgeKey] ? ` (${badges[item.badgeKey]})` : ""}
                        </span>
